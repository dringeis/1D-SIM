...
! WATCHOUT: some variables need to be passed from prep_fgmres...not done now

subroutine EVP2Cprecond (rhs, utp, zeta, eta, Cw, ts, gamma_nl, upts, uk1)
  use size
  use resolution
  use properties
  use global_var
  ...
  double precision :: u_pos(1:nx+1), zeta_pos(0:nx+1), eta_pos(0:nx+1)
...
!------------------------------------------------------------------------
! evaluate dzeta/du * du at u^k-1
!------------------------------------------------------------------------
     u_pos = uk1 + 1d-06*utp
     call viscouscoefficient (u_pos, zeta_pos, eta_pos) ! u is u^k-1
...
!------------------------------------------------------------------------
     ...
     do i = 1, nx ! for tracer points
        ...
        right = (zeta(i)/ T )*( utp(i+1) - utp(i) ) / Deltax &
              + sigma(i) / Deltate &
              + (1d06/T)*(zeta_pos(i)-zeta(i))*( uk1(i+1) - uk1(i) ) / Deltax
...
!------------------------------------------------------------------------
!     B1: rho*h*du^p-1 / Deltat
!------------------------------------------------------------------------
 
        B1 = B1 - ( rho * h_at_u(i) * utp(i) ) / Deltat
