...
subroutine EVP1Bprecond (rhs, utp, zeta, eta, Cw, ts)
  use size
  use resolution
  use properties
  use global_var
  ...
  double precision :: Cw(1:nx+1), F_uk1(1:nx+1), Epar(0:nx+1)
...
!------------------------------------------------------------------------
  ...
  do i = 1, nx ! could be improved
     ...
     Epar(i)   = A(i)*h(i)*Estar ! no change during subcycling
...
!------------------------------------------------------------------------
     ...
     do i = 1, nx ! for tracer points
        ...
        left = 1d0 + (Epar(i)*Deltate)/(zeta(i)+eta(i))

        right = sigma(i) + (Epar(i)*Deltate)*(utp(i+1) - utp(i)) / Deltax
