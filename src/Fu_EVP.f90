...
subroutine Fu_EVP (utp, us1, zeta, eta, Cw, b, Fu_vec)
  use size
  use resolution
  use properties
  use global_var
  ...
  double precision, intent(in)  :: utp(1:nx+1), us1(1:nx+1)
...
!------------------------------------------------------------------------
!    rhoice*h*du/dte : diff with regular residual calc
!------------------------------------------------------------------------
     
     Fu_vec(i) = Fu_vec(i) +  rho * h_at_u * (utp(i)-us1(i)) / Deltate
